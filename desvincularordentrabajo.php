...
<?php
  ...
  {  
    ...
    $sql="UPDATE autorizaraccorden SET estado=?,observacion=?,accion=?,idempleadoaccion=?,fechaaccion=?
          WHERE numorden=? AND idpersona=?;";

    $con=conectar();

    $sentencia=mysqli_prepare($con,$sql);//preparo consulta
    mysqli_stmt_bind_param($sentencia,'sssssss',$estado,$obs,$accion,$idusuario,$fechaaccion,$numorden,$idusuario);
