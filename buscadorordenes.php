...
<head>
  ...
  * F => TAREA TERMINADA
  ...
  <script>
    ...
    {
      //alert("Se cambia estado orden " + num + " a disponible!!!");

      organizartareas(num,'D');
    }
    ...
    {
      alert("Se muestra historial de la matricula " + num);
    }
    ...
    function verorden() {
      ...
      function organizartareas(orden,estado) {
          var xmlhttp = new XMLHttpRequest();
          xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
              //alert("Se envia la orden numero=>"+orden+" y se lo pasa al estado de =>"+estado);
              document.getElementById("lsinfo").innerHTML=this.responseText;
            }
          };
          xmlhttp.open('GET', 'organizarorden.php?orden='+orden+'&estado='+estado, false);
          xmlhttp.send();
      }
